...
class NextEventsHandler(BaseHandlerWithLogger):
    ...
    def handle_impl(self, message: Message):
        ...
        if not events:
            ...
            if len(filtered_events) == 0:
                self.bot.send_message(message.chat.id, 'There are no future events in the calendar.')
            else:
                self.bot.send_message(message.chat.id, f'Future events in the calendar:\n{event_list}')
