    ...
    def handle_impl(self, message: Message):
        if not self.event_date and self.key:
            ...
            if not current_event:
                ...
                self.bot.send_message(message.chat.id, 'Something went wrong, try to start again.')
                return
            ...
            self.bot.edit_message_text(f'Event "{event_text}" added to the calendar on {self.event_date.strftime("%d/%m/%Y")}.',
                                       message.chat.id,
                                       message.message_id)
